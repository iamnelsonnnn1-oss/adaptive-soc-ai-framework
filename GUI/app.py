...
def render_header() -> None:
    ...
    if os.path.exists(logo_path):
        with open(logo_path, "rb") as f:
            ...
            logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="height:130px;margin-right:25px;vertical-align:middle;filter:drop-shadow(0 0 10px #00FF00);">'
    ...
    header_html = f'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:30px;border-bottom:1px solid #1A1A1A;padding-bottom:20px;"><div style="display:flex;align-items:center;">{logo_html}<div><h1 style="margin:0;font-family:\'Courier New\',monospace;font-size:1.6rem;font-weight:900;letter-spacing:4px;color:#FFFFFF;">SECUREX COMMAND</h1><p style="color:#777777;margin:5px 0 0 0;font-size:0.75rem;letter-spacing:1px;">[ SYSTEM INFRASTRUCTURE MONITORING V1.0 ]</p></div></div><div style="display:flex;gap:40px;align-items:center;"><div style="text-align:right;"><div style="color:#777777;font-size:0.65rem;letter-spacing:1px;">THREATS TODAY</div><div style="color:#00FF00;font-weight:bold;font-size:1.2rem;">17</div></div><div style="text-align:right;"><div style="color:#777777;font-size:0.65rem;letter-spacing:1px;">ASSETS MONITORED</div><div style="color:#FFFFFF;font-weight:bold;font-size:1.2rem;">2,491</div></div><div style="background:#000000;border:1px solid #00FF00;padding:8px 15px;"><span class="status-pulse-commander"></span><span style="color:#00FF00;font-weight:bold;font-size:0.8rem;letter-spacing:2px;font-family:\'Courier New\',monospace;">COMMAND CENTER ACTIVE</span></div></div></div>'
...
def render_anomaly_map() -> None:
    ...
    map_html = '<div style="background:#050505;border:1px solid #1A1A1A;padding:25px;border-radius:4px;height:550px;overflow:hidden;"><svg viewBox="0 0 1000 420" style="width:100%;height:100%;"><defs><pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="#111" stroke-width="0.5"/></pattern><style>@keyframes globe-panning { 0% { transform: translateX(0); } 100% { transform: translateX(-40px); } } .panning-grid { animation: globe-panning 10s linear infinite; } @keyframes alert-cycle { 0%, 100% { opacity: 0; transform: translateY(10px); } 10%, 40% { opacity: 1; transform: translateY(0); } 50% { opacity: 0; transform: translateY(-10px); } } .map-alert { animation: alert-cycle 8s infinite; }</style></defs><rect class="panning-grid" width="1040" height="420" fill="url(#grid)" /><line x1="150" y1="150" x2="500" y2="210" class="map-connection" /><line x1="850" y1="120" x2="500" y2="210" class="map-connection" /><line x1="800" y1="350" x2="500" y2="210" class="map-connection" /><line x1="200" y1="380" x2="500" y2="210" class="map-connection" /><circle cx="150" cy="150" r="4" fill="#00FF00" /><circle cx="150" cy="150" r="4" class="map-node-pulse" style="animation-delay: 0s;" /><text x="140" y="135" fill="#777777" font-size="10">NODE-US-PROD</text><circle cx="850" cy="120" r="4" fill="#00FF00" /><circle cx="850" cy="120" r="4" class="map-node-pulse" style="animation-delay: 0.5s;" /><text x="840" y="105" fill="#777777" font-size="10">NODE-EU-CENTRAL</text><circle cx="800" cy="350" r="4" fill="#00FF00" /><circle cx="800" cy="350" r="4" class="map-node-pulse" style="animation-delay: 1.2s;" /><text x="790" y="335" fill="#777777" font-size="10">NODE-AP-SOUTH</text><circle cx="200" cy="380" r="4" fill="#00FF00" /><circle cx="200" cy="380" r="4" class="map-node-pulse" style="animation-delay: 0.8s;" /><text x="190" y="365" fill="#777777" font-size="10">NODE-LA-SOUTH</text><rect x="485" y="195" width="30" height="30" fill="none" stroke="#FFFFFF" stroke-width="1" /><text x="470" y="245" fill="#FFFFFF" font-size="12" font-weight="bold">SECUREX HUB</text><g class="map-alert"><rect x="650" y="180" width="140" height="45" fill="#0A0A0A" stroke="#00FF00" stroke-width="0.5" /><text x="660" y="200" fill="#00FF00" font-size="9" font-weight="bold">THREAT: ISOLATION</text><text x="660" y="215" fill="#777777" font-size="8">SRC: 10.42.1.204</text></g></svg></div>'
    st.markdown("<p style='color: #FFFFFF; margin: 0 0 10px 0; font-size: 0.7rem;'>// GLOBAL ANOMALY MAP</p>", unsafe_allow_html=True)
    map_html = '<div style="background:#050505;border:1px solid #1A1A1A;padding:25px;border-radius:4px;height:550px;overflow:hidden;"><svg viewBox="0 0 1000 420" style="width:100%;height:100%;"><defs><pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="#111" stroke-width="0.5"/></pattern><style>@keyframes globe-panning { 0% { transform: translateX(0); } 100% { transform: translateX(-40px); } } .panning-grid { animation: globe-panning 5s linear infinite; } @keyframes alert-cycle { 0%, 100% { opacity: 0; transform: translateY(10px); } 10%, 40% { opacity: 1; transform: translateY(0); } 50% { opacity: 0; transform: translateY(-10px); } } .map-alert { animation: alert-cycle 8s infinite; }</style></defs><rect class="panning-grid" width="1040" height="420" fill="url(#grid)" /><line x1="150" y1="150" x2="500" y2="210" class="map-connection" /><line x1="850" y1="120" x2="500" y2="210" class="map-connection" /><line x1="800" y1="350" x2="500" y2="210" class="map-connection" /><line x1="200" y1="380" x2="500" y2="210" class="map-connection" /><circle cx="150" cy="150" r="4" fill="#00FF00" /><circle cx="150" cy="150" r="4" class="map-node-pulse" style="animation-delay: 0s;" /><text x="140" y="135" fill="#FFFFFF" font-size="10">NODE-US-PROD</text><circle cx="850" cy="120" r="4" fill="#00FF00" /><circle cx="850" cy="120" r="4" class="map-node-pulse" style="animation-delay: 0.5s;" /><text x="840" y="105" fill="#FFFFFF" font-size="10">NODE-EU-CENTRAL</text><circle cx="800" cy="350" r="4" fill="#00FF00" /><circle cx="800" cy="350" r="4" class="map-node-pulse" style="animation-delay: 1.2s;" /><text x="790" y="335" fill="#FFFFFF" font-size="10">NODE-AP-SOUTH</text><circle cx="200" cy="380" r="4" fill="#00FF00" /><circle cx="200" cy="380" r="4" class="map-node-pulse" style="animation-delay: 0.8s;" /><text x="190" y="365" fill="#FFFFFF" font-size="10">NODE-LA-SOUTH</text><rect x="485" y="195" width="30" height="30" fill="none" stroke="#FFFFFF" stroke-width="1" /><text x="470" y="245" fill="#FFFFFF" font-size="12" font-weight="bold">SECUREX HUB</text><g class="map-alert"><rect x="650" y="180" width="140" height="45" fill="#0A0A0A" stroke="#00FF00" stroke-width="0.5" /><text x="660" y="200" fill="#00FF00" font-size="9" font-weight="bold">THREAT: ISOLATION</text><text x="660" y="215" fill="#FFFFFF" font-size="8">SRC: 10.42.1.204</text></g></svg></div>'
    st.markdown(map_html, unsafe_allow_html=True)


def render_ai_analyst() -> None:
    ai_html = '<div class="ai-analyst-box"><div style="color:#00FF00;font-weight:bold;font-size:0.9rem;margin-bottom:10px;">🤖 AI CHARLIE SOC ANALYST</div><div style="display:flex;gap:30px;"><div style="flex:1;"><p style="color:#FFFFFF;font-size:0.85rem;line-height:1.6;">"Behavioral anomaly detected from Kubernetes worker node 3. Cross-referencing with network logs indicates potential data staging in unauthorized S3 bucket."</p></div><div style="width:200px;border-left:1px solid #1A1A1A;padding-left:20px;"><div style="color:#777777;font-size:0.65rem;">RISK SCORE</div><div style="color:#00FF00;font-size:1.5rem;font-weight:bold;">82/100</div><div style="margin-top:10px;color:#FFFFFF;font-size:0.7rem;font-weight:bold;">RECOMMENDATION:<br><span style="color:#777777;font-weight:normal;">Isolate Node 3 & Block egress to bucket audit-exfil-demo.</span></div></div></div></div>'
