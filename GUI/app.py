...
def render_active_threats() -> None:
    ...
    for _, row in threats.iterrows():
        severity_color = {"Critical": "#FF003C", "High": "#FFFFFF", "Medium": "#777777", "Low": "#444444"}.get(row["Severity"], "#222222")
        st.markdown(f"""
        <div style="margin-bottom: 12px; border-left: 2px solid {severity_color}; padding-left: 10px;">
            <div style="font-size: 0.75rem; color: #777777;">[{row['Time']}] <span style="color: {severity_color};">{row['Source']}</span></div>
            <div style="font-size: 0.8rem; color: #FFFFFF; font-weight: bold;">{row['Vector']}</div>
        </div>
        """, unsafe_allow_html=True)
...
def render_anomaly_map() -> None:
    ...
    st.markdown("""
    <div style="background: #050505; border: 1px solid #1A1A1A; padding: 20px; border-radius: 4px;">
        <svg viewBox="0 0 800 350" style="width: 100%;">
            <line x1="150" y1="100" x2="400" y2="180" class="map-connection" />
            <line x1="600" y1="80" x2="400" y2="180" class="map-connection" />
            <line x1="650" y1="280" x2="400" y2="180" class="map-connection" />
            <circle cx="150" cy="100" r="4" fill="#FF003C" />
            <circle cx="150" cy="100" r="4" class="map-node-pulse" />
            <text x="140" y="85" fill="#777777" font-size="10">USA-WEST-01</text>
            <circle cx="600" cy="80" r="4" fill="#FF003C" />
            <circle cx="600" cy="80" r="4" class="map-node-pulse" />
            <text x="590" y="65" fill="#777777" font-size="10">EU-CENTRAL-1</text>
            <circle cx="650" cy="280" r="4" fill="#FF003C" />
            <circle cx="650" cy="280" r="4" class="map-node-pulse" />
            <text x="640" y="265" fill="#777777" font-size="10">APAC-SOUTH-02</text>
            <rect x="385" y="165" width="30" height="30" fill="none" stroke="#FFFFFF" stroke-width="1" />
            <text x="380" y="215" fill="#FFFFFF" font-size="12" font-weight="bold">SOC CORE</text>
            <rect x="520" y="150" width="120" height="40" fill="#0A0A0A" stroke="#FF003C" stroke-width="0.5" />
            <text x="530" y="165" fill="#FF003C" font-size="9" font-weight="bold">ANOMALY DETECTED</text>
            <text x="530" y="180" fill="#777777" font-size="8">IP: 192.168.1.42</text>
        </svg>
    </div>
    """, unsafe_allow_html=True)
...
def render_ai_analyst() -> None:
    ...
    <div class="ai-analyst-box">
        <div style="color: #FF003C; font-weight: bold; font-size: 0.9rem; margin-bottom: 10px;">🤖 AI CHARLIE SOC ANALYST</div>
        <div style="display: flex; gap: 30px;">
            <div style="flex: 1;">
                <p style="color: #FFFFFF; font-size: 0.85rem; line-height: 1.6;">
                    "Behavioral anomaly detected from Kubernetes worker node 3. 
                    Cross-referencing with network logs indicates potential data staging 
                    in unauthorized S3 bucket."
                </p>
            </div>
            <div style="width: 200px; border-left: 1px solid #1A1A1A; padding-left: 20px;">
                <div style="color: #777777; font-size: 0.65rem;">RISK SCORE</div>
                <div style="color: #FF003C; font-size: 1.5rem; font-weight: bold;">82/100</div>
                <div style="margin-top: 10px; color: #FFFFFF; font-size: 0.7rem; font-weight: bold;">
                    RECOMMENDATION:<br><span style="color: #777777; font-weight: normal;">Isolate Node 3 & Block egress to bucket 'audit-exfil-demo'.</span>
                </div>
            </div>
        </div>
    </div>
...
def render_pipeline_status() -> None:
    ...
    for _, row in pipelines.iterrows():
        card_html = f'<div class="pipeline-card"><div class="pipeline-name">{row["Pipeline"]}</div><div style="color:#FF003C;font-size:0.75rem;font-weight:bold;margin-top:4px;">● SECURE</div></div>'
